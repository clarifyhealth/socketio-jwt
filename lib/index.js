...
function authorize(options, onConnection) {
  ...
  return function(data, accept){
    ...
    console.log('SOCKETIO-JWT: req.client.localAddress: %s', req.client.localAddress);
    if (req.client.localAddress === '127.0.0.1' || req.client.localAddress === 'localhost') {
        return auth.success(data, accept);
    }

    console.log('SOCKETIO-JWT: authorization_header: %s', authorization_header);
    ...
    console.log('SOCKETIO-JWT: token: %s', token);
...
function getSecret(request, secret, token, callback) {
  console.log('SOCKETIO-JWT: getSecret: request: %s', request);
  console.log('SOCKETIO-JWT: getSecret: token: %s', token);
  if (typeof secret === 'function') {
    if (!token) {
      console.log('SOCKETIO-JWT: getSecret: error: %s', 'jwt must be provided');
    ...
    if (parts.length < 3) {
      console.log('SOCKETIO-JWT: getSecret: error: %s', 'jwt malformed');
      return callback({ code: 'invalid_token', message: 'jwt malformed' });
    }

    if (parts[2].trim() === '') {
      console.log('SOCKETIO-JWT: getSecret: error: %s', 'jwt signature is required');
